...
<script type="text/javascript">
	function search() {
		...
		window.location.href = "${ctx}/user/list1?searchParam=" + p;
		$("[name='pagen']").each(
				function(key, value) {
					$(this).attr("href",
			...
			<div class="row ">
				...
				<div class="col-xs-10">
					...
					<table
						...
						<tbody>
							<c:forEach items="${page1.list}" var="u">
								<tr>
									<td>${u.userNumber}</td>
									<td>${u.loginName}</td>
									<td>${u.userName}</td>
									...
									<td>${u.salary}</td>
									<td>${u.tel}</td>
									<td><a href="${ctx }/user/edit1?userNumber=${u.userNumber}"
										class="btn btn-primary btn-sm" data-toggle="modal"
										data-target="#edit">修改</a> <a
										href="${ctx }/user/delete1?userNumber=${u.userNumber}"
									...
									<div class="modal fade" id="edit" tabindex="-1" role="dialog"
										...
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												...
												<form action="${ctx }/user/edit1?userNumber=${u.userNumber}"
													method="post">
													<div class="modal-body">

														<ul>
															<li><label><span>员工工号：</span></label> <input
																type="text" value="${u.userNumber }" name="name" /></li>
															<li><label><span>员工登录名 ：</span></label> <input
																type="text" value="${u.loginName }" name="price" /></li>
															<li><label><span>真实姓名：</span></label> <input
																type="text" value="${u.userName}" name="stock" /></li>
															<li><label><span>入职时间：</span></label> <input
																type="text" value="${u.entryTime }" name="place" /></li>
															<li><label><span>薪资 ：</span></label> <input
																type="text" value="${u.salary}" name="time" /></li>
															<li><label><span>联系方式：</span></label> <input
																type="text" value="${u.tel}" name="nature" /></li>															
														</ul>
													</div>
					...
					<ul class="pagination right">
						...
						<c:forEach begin="1" end="${page1.totalPageNum }" var="pageNum">
							<li class="active"><a name="pagen"
								href="${ctx }/user/list?pageNum=${pageNum}">${pageNum}</a></li>
						</c:forEach>
